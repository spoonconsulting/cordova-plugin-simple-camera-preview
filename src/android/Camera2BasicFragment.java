...
public class Camera2BasicFragment extends Fragment implements ActivityCompat.OnRequestPermissionsResultCallback {
    ...
    private final ImageReader.OnImageAvailableListener mOnImageAvailableListener
        ...
        public void onImageAvailable(ImageReader reader) {
            ...
            mBackgroundHandler.post(new ImageSaver(reader.acquireNextImage(), mFile, (File savedImage) -> {
                Log.d(TAG, "saved image to " + mFile.toString());
                if (takePictureCallback != null) {
                    takePictureCallback.onCompleted(null, mFile.getName());
                }
            }));
        }
...
//        view.findViewById(R.id.info).setOnClickListener(this);
    ...
    private void configureTransform(int viewWidth, int viewHeight) {
        ...
        if (getScreenOrientation() == Configuration.ORIENTATION_LANDSCAPE)
            xShift = (viewHeight - screenWidth) / 2;
...
//      int jpegOrientation =  (sensorOrientation + deviceOrientation + 360) % 360;
            ...
            captureBuilder.set(CaptureRequest.JPEG_ORIENTATION, getOrientation(deviceRotation));
            //captureBuilder.set(CaptureRequest.JPEG_ORIENTATION, getOrientation(ORIENTATION_PORTRAIT));
    ...
    private static class ImageSaver implements Runnable {
        ...
        public interface ImageSaverCallback {
            void didSaveImage(File file);
        }

        private ImageSaverCallback callback;
        ...
        ImageSaver(Image image, File file, ImageSaverCallback saveCallback) {
            mImage = image;
            mFile = file;
        ...
        @Override
        public void run() {
            Log.d(TAG, "saving image to " + mFile.toString());
            ...
            try {
                ...
                output.write(bytes);
                callback.didSaveImage(mFile);
            } catch (IOException e) {
                e.printStackTrace();
                callback.didSaveImage(null);
            } finally {
                mImage.close();
                if (null != output) {
                    try {
